...
* _Highly concurrent._ Axum has its own Tokio thread pool internally, which provides concurrency on the request level; internally, a [Rayon](https://github.com/rayon-rs/rayon) thread pool is created on server startup (of configurable size) and each thread gets its own highlighter, providing concurrency on the file level. (This eliminates Cap'n Proto RPC as an option, as its server is single threaded and it is not possible to pass capnp `Reader` values over a thread boundary without copying, so a thread pool would be problematic.)
